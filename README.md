...
> The project version mirrors the extension version its patch signatures were verified against: **2.1.276**.
...
- The `anthropic.claude-code` extension installed (verified against **2.1.276**; the same signatures also match 2.1.274, the only other bundle still on disk — every release before it keeps its own `v<version>` branch)
...
injection point #9 already sits, on the list the app is about to render. That list is also the one it
...
reassigns the memo. A single anchor spanning both would have overlapped the filter's own match.

That release also gave the search row two filters of its own — an **Active** toggle and a **Filter by
status** menu over `needs_input` / `working` / `completed`. They subtract rows; the sort only reorders
what survives, so they compose the same way search does. One consequence worth knowing: a pinned
session the stock filter excludes is not shown at all, because the filter runs first.
...
|---|---|---|
| `main` | 2.1.276 — newest | `M.env=ye$(E,H===!0);`, `light:K,dark:K` |
| `v2.1.276` | 2.1.276 | `M.env=ye$(E,H===!0);`, `light:K,dark:K` |
| `v2.1.274` | 2.1.274 | `M.env=Ba$(E,H===!0);`, `light:G,dark:G` |
| `v2.1.273` | 2.1.273 | `Z.env=R;`, `light:W,dark:W` |
| `v2.1.272` | 2.1.272 | `Z.env=R;`, `light:W,dark:W` |
| `v2.1.269` | 2.1.269 | `Z.env=R;`, `light:W,dark:W` |
| `v2.1.268` | 2.1.268 | `Z.env=R;`, `light:W,dark:W` |
| `v2.1.267` | 2.1.267 | `O.env=R;`, `light:W,dark:W` |
| `v2.1.266` | 2.1.266 | `N.env=M;`, `light:W,dark:W` |
| `v2.1.259` | 2.1.259 | `N.env=F;`, `light:K,dark:K` |
| `v2.1.258` | 2.1.258 | `N.env=L;`, `light:W,dark:W` |
...
One commit can serve several versions, and each branch differs from `main` only by its version stamp — the code is the same because injection points #2 and #3 match the *shape* of the assignment rather than the names in it. Both structural signatures are re-checked against every bundle still on disk at update time. VS Code garbage-collects retired extension folders, so the older ones eventually stop being verifiable locally — 2.1.220 through 2.1.226 were gone by the 2.1.228 update, and by 2.1.233 the sweep was 2.1.227 through 2.1.233 — at the 2.1.239 update only 2.1.235, 2.1.238 and 2.1.239 were still there, 2.1.241 joined that set without retiring any of it, the 2.1.245 update finally swept the rest, and by 2.1.247 the set is 2.1.245, 2.1.246 and 2.1.247 — that update retired the first two without deleting them, so all three were still verifiable, and 2.1.250 joined them the same way, leaving all four on disk — 2.1.251 makes five, 2.1.252 six, 2.1.257 seven and 2.1.258 eight, none of them retiring any of the others — the 2.1.259 update finally swept most of that set, leaving 2.1.245, 2.1.246, 2.1.247, 2.1.250, 2.1.251, 2.1.252, 2.1.257, 2.1.258 and 2.1.259 on disk with only the last one active — and by 2.1.266 that set is down to three folders, 2.1.259, 2.1.265 and 2.1.266, of which only two can be checked: 2.1.265 was retired without ever being adapted to, and its `webview/index.js` is already deleted while the rest of the folder lingers — the 2.1.267 update swept both of those, leaving just 2.1.266 and 2.1.267, and the 2.1.268 update swept 2.1.266 in turn, leaving 2.1.267 and 2.1.268 with only the latter active — and the 2.1.269 update swept both of those, leaving one folder in the tree and no `.obsolete` file at all, which is the first single-folder sweep in this table and the point at which nothing older can be re-checked locally any more. The 2.1.272 update then went the other way and swept nothing: it marked 2.1.269 obsolete in `.obsolete` and left the folder whole, so both bundles could be compared, and 2.1.273 did the same to 2.1.272 — three folders on disk now, 2.1.269, 2.1.272 and 2.1.273, with only the last active. 2.1.274 continued that, marking 2.1.273 obsolete and leaving its folder whole: four folders, three of them still verifiable. 2.1.269 was the first entry here that was still on disk and still no longer verifiable: 2.1.272 widened injection point #10 to take a block body, and 2.1.269's bare-arrow handler does not have one, so the signature `main` carries scored zero hits against it. That is the one place where a version branch differs from `main` by more than its version stamp. 2.1.276 then swept everything older than 2.1.274 — two folders on disk now, 2.1.274 and 2.1.276, both of them verifiable, and the first sweep since 2.1.269 to actually delete anything. Their branches stay pinned to the commit that was verified against them.

Renames have now cost almost every signature at least once, and 2.1.245 is where they reached the two that had held out longest. 2.1.238 renamed the session list's `useRef` alias (`ge` → `_e`), the one hook alias injection point #6 still hardcoded. 2.1.239 renamed a helper injection point #4 reads through to `$b` — and since that signature spelled its identifiers `\w+`, which does not match a `$`, it fell to zero hits on a name it was not even capturing. 2.1.245 carried that same `$` into `extension.js` for the first time and took #1 and #2 with it: #2 spelled its locals `\w+` while the panel had become `$`, and #1 was still a plain literal naming three locals outright — the nonce, the bundle's own script URI, and the webview parameter the injected call is handed. All of them are fixed the same way, by capturing what was being spelled out; #1 now anchors on `getHtmlForWebview`, the one unminified name within reach. 2.1.227 is the other kind of break — it dropped the bundled-node fallback, which deleted the `if(…)` wrapper around #3 and left the assignment ending in `;` instead of `,<nodePath>)`. The signature now captures that terminator and echoes it back verbatim, so one pattern still covers every release in the table. 2.1.241 is the counter-example worth recording: all eight signatures matched it untouched, the first release since 2.1.234 to cost nothing at all. 2.1.246 is the second, and the more surprising one — it is the release that moved the entire model catalogue out of `extension.js` and into the CLI binary, and still every signature matched as written, because none of them reads that catalogue. 2.1.247 makes three in a row: its whole diff is git-invocation hardening, a usage-limit grace banner behind an experiment gate, and two spinner-tips settings, and no signature stands on any of it. 2.1.250 and 2.1.251 take that to five, and 2.1.252 to six. 2.1.257 ends the run: it moved what #7 was assigning without moving the shape being matched — the app stopped rendering the filter’s result and started rendering a memo derived from it — so the sort was split off into its own point #8, which is why there are now nine. 2.1.251 did rename two locals inside #3 (`N.env=F;` → `N.env=D;`), which is precisely the rename the structural signature exists to absorb — it cost nothing, and the same is true of every release in this paragraph that is not named as a break. 2.1.252 did not even do that: its two bundles are the same length as 2.1.251's to the byte and differ in seven single-character runs, every one of them a version string, so not one identifier moved. 2.1.258 is that release again, to the shape of it — same lengths, same seven runs, same absence of any identifier move — and it starts a fresh run of releases that cost nothing. 2.1.259 ends that run after one: it rebuilt the session-list component around a status filter and cost three signatures at once — #6 gained two declarations between the pair it brackets, #7 had its lower-cased query hoisted out of the callback, and #8 lost the accessor it captured to a spot above the memo it sorts, which split it into #8 and #9 and took the injection count to ten. 2.1.266 is a kind of break with no precedent here at all: not a rename, but a new *argument shape*. The helper that builds #4's model label used to be handed the selection raw and now receives it resolved through the config first — `Uy(sel, …)` became `Uy(N_(cfg,sel)?.value??sel, …)` — and since the signature spelled that argument as a back-reference to the selection local, it fell to zero hits over a value it does not capture and does not use. It now matches the argument as `[^;]*?`, which a minified expression cannot contain. The other nine matched as written, #3 included, across two renames (`N.env=F;` → `N.env=M;` and `light:K` → `light:W`). 2.1.267 costs nothing: all ten signatures match it as written, through a third #3 rename (`N.env=M;` → `O.env=R;`) that the structural signature absorbs, and its whole diff is one CLI setting (`maxEffortLevel`) plus two strings. 2.1.268 costs nothing either, through a fourth rename inside #3 (`O.env=R;` → `Z.env=R;`) that the structural signature absorbs again. Its bundles grew by 129,986 and 35,351 bytes, but almost none of that is feature code: `extension.js` gained not one new readable string, and both growths are re-vendored dependencies (an HTTP client with `FormData`, `debug`/`supports-color`, a `whsec_` webhook verifier) plus a re-minified React in the webview. What the release actually adds is one VS Code command (`claude-vscode.focusLastMessage`), three CLI settings (`prependPlugins`, `appendPlugins`, `gatewayInternalNetworks`), no new webview actions and no new VS Code settings. 2.1.269 costs nothing either, and for the first time since 2.1.266 not even a rename to absorb: #3 is still `Z.env=R;` and the panel icon still `light:W,dark:W`, so both structural signatures match the same text they matched a release ago. The Model section's sort order is still the stock five, byte for byte, so nothing of theirs has joined the section this patch inserts into. What is *not* recorded for it is a diff: this update swept both older folders and left 2.1.269 alone on disk, so there was no bundle to compare against — everything said about it here is read off 2.1.269 itself. 2.1.272 ends that run with the one break in this paragraph that is neither a rename nor an argument shape: the search input's `onChange` body grew a second statement, `(X1)=>i1(X1.target.value)` became `(Y1)=>{X0(Y1.target.value),I2(!0)}`, and #10 — which spelled out the single call it expected — fell to zero hits. It now captures the block as an opaque statement list and re-emits it verbatim, so the next statement they add inside rides along; the cost is that a nested `{…}` in there would end the match early and fail loudly. 2.1.273 costs nothing, and is the third release running where neither structural anchor moved — #3 still `Z.env=R;`, #2 still `light:W,dark:W`. Its diff is the thinnest kind recorded here that is not a pure version bump: no new commands, no new VS Code settings, no new CLI settings keys, no new webview actions, and not one new human-readable string in either bundle. The bundles grew by 9,713 and 1,218 bytes and the CLI binary by 1,547,776; what `extension.js` actually gained is a sandbox-policy merge (`isolation.persistHome`, `sandbox.egress.allowedHosts`/`deniedHosts`) and a `command_lifecycle` tracker over `queued`/`started`/`completed`/`cancelled`/`discarded`/`refused`. Neither is reachable from anything this project touches, and neither duplicates it: the egress lists gate a sandbox's network, not which endpoint a session talks to. 2.1.274 ends that run with a break of the one kind #3 had never seen: the env being assigned stopped being a local and became an **expression**. `Z.env=R;` became `M.env=Ba$(E,H===!0);`, where that helper adds `CLAUDE_CODE_RESUME_INTERRUPTED_TURN=1` (max age 3,600,000 ms) when `spawnClaude`'s new thirteenth parameter says the turn is being resumed after an interrupt — the `claudeCode.continueAfterReload` setting this release adds, on by default. The signature captured the env as a bare identifier, so it fell to zero hits; it now allows one trailing call with no nesting, `[w$]+(?:([^()]*))?`, and `envFor` receives the helper's result rather than the raw env. The other nine matched as written, through a rename of #2's pair (`light:W,dark:W` → `light:G,dark:G`) that the structural signature absorbs. The rest of the release does not reach this project: a second new VS Code setting (`claudeCode.lockEditorGroups`, on by default, which locks the editor group a Claude tab opens in), two new webview actions (`memory` and `instructions`, a dialog over the project's CLAUDE.md files), no new commands at 27, and a CLI settings schema unchanged but for its `$comment` timestamp. `continueAfterReload` is the one worth a second look, since claudapter also offers to resume after a halt — but it fires on a window reload and works by handing the CLI an env variable, while claudapter's prompt fires on an error, a usage limit or an interrupt and only fills the composer, so the two never trigger on the same event. 2.1.276 costs nothing, and it is the case both structural anchors exist for: they renamed at once. #3's env helper went `Ba$(E,H===!0)` → `ye$(E,H===!0)` and #2's icon pair `light:G,dark:G` → `light:K,dark:K`, and all fifteen signatures matched as written through both. 2.1.275 never landed on this machine. The release itself is editor-side: `contributes.commands` goes 27 → 31 over an inline accept/reject gutter for a proposed edit (`acceptProposedHunk`, `rejectProposedHunk` and their `…FromBar` variants), VS Code settings 19 → 20 (`claudeCode.scrollToBottomOnSend`), and the CLI settings schema holds at 168 top-level keys while gaining `version` and `registry` under each of `allowedMarketplaces`, `blockedMarketplaces` and `strictKnownMarketplaces`. The webview still registers 27 actions and the Model section's sort order is still the stock five, one hit in the stock backup. What the webview actually gained is a rewind that restores code as well as the conversation — the dialog titles itself "Rewind code", offers a "What to restore" choice group, and reports "Code rewind completed, but N files were skipped". That is the one thing here worth a second look, since claudapter also lets a turn be taken back — but retract talks to the session object, hides the failed exchange and instructs the agent to try again, while this one rolls files back to a checkpoint. They are armed from different places and neither can see the other.
