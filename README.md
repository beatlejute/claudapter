...
> The project version mirrors the extension version its patch signatures were verified against: **2.1.250**.
...
- The `anthropic.claude-code` extension installed (verified against **2.1.250**; the signatures also match 2.1.220–2.1.247)
...
|---|---|---|
| `main` | 2.1.250 — newest | `N.env=F;`, `light:W,dark:W` |
...
One commit can serve several versions, and each branch differs from `main` only by its version stamp — the code is the same because injection points #2 and #3 match the *shape* of the assignment rather than the names in it. Both structural signatures are re-checked against every bundle still on disk at update time. VS Code garbage-collects retired extension folders, so the older ones eventually stop being verifiable locally — 2.1.220 through 2.1.226 were gone by the 2.1.228 update, and by 2.1.233 the sweep was 2.1.227 through 2.1.233 — at the 2.1.239 update only 2.1.235, 2.1.238 and 2.1.239 were still there, 2.1.241 joined that set without retiring any of it, the 2.1.245 update finally swept the rest, and by 2.1.247 the set is 2.1.245, 2.1.246 and 2.1.247 — that update retired the first two without deleting them, so all three were still verifiable, and 2.1.250 joined them the same way, leaving all four on disk. Their branches stay pinned to the commit that was verified against them.
