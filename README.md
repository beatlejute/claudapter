...
> The project version mirrors the extension version its patch signatures were verified against: **2.1.273**.
...
- The `anthropic.claude-code` extension installed (verified against **2.1.273**; the same signatures also match 2.1.272, the only other bundle on disk they still fit — 2.1.269 is there too but no longer matches injection point #10, so it keeps its own `v<version>` branch, as does every release before it)
...
|---|---|---|
| `main` | 2.1.273 — newest | `Z.env=R;`, `light:W,dark:W` |
