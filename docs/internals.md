...
A teardown of version **2.1.233** (`anthropic.claude-code-2.1.233-win32-x64`); still current through **2.1.276**. 2.1.234's bundle was byte-for-byte the same shape. 2.1.235's was not — the session list's `useState` alias moved from `ne` to `ie` — and neither was 2.1.238's, which renamed the same component's `useRef` alias from `ge` to `_e` (both under "Content search reuses the session-list handoff" below). 2.1.239 broke a third — injection point #4, on the `$`-in-an-identifier trap that #6–#10 already carry a warning about (see "The session is not reachable from the context object"). Everything else held: all eight signatures match 2.1.239 with those captures generalised and nothing else touched, and 2.1.241 needed no change at all — every signature matched it as written, and its own diff against 2.1.239 is an onboarding-checklist entry and two strings. 2.1.245 broke two more, for a reason with no precedent in this file: it is the first release to put `$`-prefixed names into `extension.js`, which cost #2 its `\w+` identifier classes and #1 its literal anchor outright (see "CSP and loading your own script"). 2.1.246 cost nothing again, which is worth saying out loud because it is the release that deleted the whole model catalogue from `extension.js` — `provider_ids`, `knowledge_cutoff`, `effort_cost_index` and every `display_name` are gone from the extension and live only in the CLI binary now (see "The CLI does not remap `claude-fable-5`"). Nothing here reads them, so nothing moved. 2.1.247 cost nothing either — three releases in a row now — and its own diff is git-invocation hardening, a usage-limit grace banner behind an experiment gate (see "The usage-limit banner has a second tenant") and two spinner-tips settings. 2.1.250 cost nothing either — four in a row — and its diff is thinner still: one CLI setting (`desktopSessionCleanupPeriodDays`), one string in `extension.js` ("telemetry is off") and one new webview block that recognises the CLI's own model-switch notices — "Set model to ", "Kept model as ", "Current model: ", "Fast mode ON · model set to" and two cloud-switch failures — so it can strip their backticks before rendering. That block reads the transcript text claudapter also reads, but it only reformats stock messages; nothing it touches is written by this project. 2.1.251 cost nothing either — five in a row — and is thinner again: no new commands, VS Code settings, CLI settings keys or webview actions, one new string in `extension.js` (`"Private Key"`, the label on a `private-key` rule in the secret scanner) and none at all in the webview. Injection point #3 renamed two of its locals (`N.env=F;` → `N.env=D;`), which the structural signature absorbs without a change. 2.1.252 cost nothing either — six in a row — and is the thinnest release recorded here: both bundles are the same length as 2.1.251's to the byte and differ in seven single-character runs, every one of them a version string (six in `extension.js`, including the SDK's own `0.3.251` → `0.3.252`, and one in the webview's footer). Not an identifier moved, and the settings schema differs only in its `$comment` generation timestamp. The whole release is the CLI binary, 46,592 bytes larger, whose only new settings key is `scratchpadDirectory` — an environment-block field naming a temp directory for the agent to use instead of `/tmp` (see "The CLI does not remap `claude-fable-5`"). 2.1.257 ends that run and is the largest release recorded here since 2.1.245: archived sessions, unread markers, session groups reachable from the tab, an output-style picker and a slash-command browser, three new commands (`markSessionUnread`, `renameSessionTab`, `addSessionTabToGroup`), two new webview actions (`output-style`, `browse-slash-commands`) and four new CLI settings (`timeFormat`, `timeZone`, `permissions.blockReadsOutsideWorkingDirectories`, `modelPicker.options[].behavesAs`). It cost exactly one signature — the spanning one, #7 — and split it in two (see "Pinning is a sort, not a DOM move"), taking the injection count from eight to nine. It is also the first release to ship something claudapter already does: the session list now partitions open sessions to the front by itself. 2.1.258 is the opposite kind of release, and an exact repeat of 2.1.252: both bundles are the same length as 2.1.257's to the byte and differ in seven single-character runs, every one of them a version string — six in `extension.js`, including the SDK's own `0.3.257` → `0.3.258`, and one in the webview's footer — while the settings schema differs only in its `$comment` generation timestamp. Not an identifier moved, and all nine signatures matched as written. 2.1.259 ends that run after a single release, and it is the second-largest break recorded here: it rebuilt the session-list component around a status filter — an "Active · N" toggle over `needs_input`/`working`/`completed` and a multi-select menu beside it — and three signatures went with it. #6 had two declarations slid between the pair it brackets (the filter state and the predicate derived from it), #7 had its lower-cased query hoisted out of the filter callback, and #8 lost the accessor it captured, which moved to *above* the memo it sorts with #7 in between; that split it into two points and took the count from nine to ten. Nothing else moved: no new commands, no new VS Code settings, no new webview actions, one new CLI settings key (`managedMcpServers`), and every extension.js signature matched as written. 2.1.266 is the first break here that is neither a rename nor a deletion but a new *argument shape*: the helper building #4's model label used to be handed the selection raw and now receives it resolved through the config first — `Uy(sel, session.lastServedModel.value, cfg)` became `Uy(N_(cfg,sel)?.value??sel, …)`. The signature spelled that argument as a back-reference to the selection local, so it fell to zero hits over a value it neither captures nor uses; it now matches the argument as `[^;]*?` (see "The session is not reachable from the context object"). The other nine matched as written, #3 included, across two renames (`N.env=F;` → `N.env=M;` and the panel icon `light:K` → `light:W`). Its own diff is small: one VS Code setting (`claudeCode.archiveInactiveSessions`), two CLI settings (`bashOutputMaxChars`, `taskOutputMaxChars`), a "Clear search" button in the session list, and an add/remove-server dialog for MCP in the webview — the first stock UI for registering an MCP server, which is the path `npm run mcp:install` takes today. It registers servers; it does not provide one, so nothing this project adds is duplicated by it. 2.1.267 costs nothing — all ten signatures match it as written, through a third rename inside #3 (`N.env=M;` → `O.env=R;`) — and its diff is the smallest since 2.1.258: no new commands, no new VS Code settings, no new webview actions, one new CLI setting (`maxEffortLevel`, a client-side effort cap that clamps `/effort`, `--effort` and `CLAUDE_CODE_EFFORT_LEVEL` on every provider, with a per-model form under `modelSettings.<model>`) and one changed string in each bundle. The cap reads nothing this project writes: claudapter surfaces the effort in the composer chip and passes one to a delegated agent, but sets no ceiling, so the two do not overlap. 2.1.268 costs nothing either — all ten signatures match it as written, through a fourth rename inside #3 (`O.env=R;` → `Z.env=R;`), while #2 did not move at all (`light:W,dark:W`). Its byte growth is the largest recorded here (129,986 in `extension.js`, 35,351 in the webview) and almost none of it is feature code: `extension.js` gained not a single new readable string, and both bundles are dominated by re-vendored dependencies (an HTTP client carrying `FormData`, `debug`/`supports-color`, a `whsec_` webhook verifier) and a re-minified React. The real diff is one VS Code command (`claude-vscode.focusLastMessage`, which reveals a chat surface and focuses its last message), three CLI settings (`prependPlugins` and `appendPlugins`, which order managed plugin hooks, and `gatewayInternalNetworks`, IPv4 CIDR blocks naming where an organization's Cloud gateway sits so `/login` can reach it), no new webview actions and no new VS Code settings. Two webview additions sit next to this project without overlapping it. The session now carries `modelSelectionProvisional` alongside `modelSelection`, and `adoptCliReportedModel` settles it against the model the CLI names in `system/init` — the selection is marked provisional until the CLI confirms it. Every field the composer chip reads (`modelSelection`, `lastServedModel`, `currentMainLoopModel`, `effortLevel`) is still declared on that class, and the number of places clearing `lastServedModel` is unchanged at four, so neither the chip nor `forgetServedModel()` is touched. The other is generated session titles (`wantsGeneratedTitle`, `applyGeneratedTitle`, `titleEpoch`), which this project does not do. `gatewayInternalNetworks` is the one key worth a second look, since claudapter also concerns itself with where a request goes — but it gates which gateway `/login` will authenticate through, not which endpoint a session talks to, so the two do not meet. So the rest was not re-verified line by line. 2.1.269 costs nothing either, and costs no rename to absorb either — the first release since 2.1.266 where neither identifier moved: #3 is still `Z.env=R;`, #2 still `light:W,dark:W`, and all ten signatures match the text they matched a release ago. The evidence is the bundle rather than a fresh run: every injected marker is present in the patched pair (`renderScript`, `attachPanel`, `envFor` in `extension.js`; `onRegistry`, `onSearchState`, `onPinState`, `pinSort`, `onSearchQuery`, `__ccxOpenState`, `__ccxSearchCandidates` and the three `ccx-` action ids in the webview), and the patcher run recorded at 2026-09-12T08:23:44.527Z reports `ccx-result: up-to-date` against it. The Model section's sort order is still the stock five — `["model","effort-level","toggle-thinking","switch-models-on-flag","account-usage"]`, one hit in the stock backup — so nothing of theirs has joined the section injection point #5 reorders, and the webview registers twenty-five actions in all. No diff was taken: this update swept both 2.1.267 and 2.1.268 and left 2.1.269 alone in `~/.vscode/extensions`, with no `.obsolete` file, so no older bundle survived to compare against, and what changed inside the release is not recorded here. 2.1.272 ends that run, and its break is a third kind again — neither a rename nor an argument shape but a **statement added to a body**: the session search input's `onChange` went from the bare call `(X1)=>i1(X1.target.value)` to a block with a second setter in it, `(Y1)=>{X0(Y1.target.value),I2(!0)}`, and point #10, which spelled out the one call it expected, matched zero times (see "Content search reuses the session-list handoff"). The signature now takes the block as an opaque `[^{}]*` statement list and re-emits it verbatim rather than rebuilding it from a captured setter name, which is why it is the only signature here that captures a *body*. The other nine matched as written; #3 was still `Z.env=R;` and the panel icon still `light:W,dark:W`, the second release running for both. 2.1.270 and 2.1.271 never landed on this machine. 2.1.273 costs nothing — all ten signatures match it as written, and for the third release running neither structural anchor moved. Its diff is the thinnest recorded here that is not a pure version bump: `contributes.commands` and `contributes.configuration` are unchanged at 27 and 17 entries with no additions or removals, the CLI settings schema holds at 293 keys and differs only in its `$comment` generation timestamp (`2026-09-14T22:57:10.478Z` → `2026-09-15T17:09:04.693Z`), the webview registers the same twenty-five actions, and neither bundle gained a single new human-readable string. The Model section's sort order is still the stock five, one hit in the stock backup. What did change is small and out of reach: `extension.js` grew 9,713 bytes over one new bundled module, and the only new identifier-level strings in it are a sandbox-policy merge (`isolation.required`, `isolation.persistHome`, and `sandbox.egress` gaining `allowedHosts`/`deniedHosts`, merged restrictive-wins) and a `command_lifecycle` tracker that holds `command_uuid`s in a set between `queued` and `started`/`completed`/`cancelled`/`discarded`/`refused`. The webview's 1,218 bytes are re-minification noise — three added strings, all of them symbol descriptions (`react.lazy`, `react.memo`, `editor.modesRegistry`). The CLI binary is 1,547,776 bytes larger and is the bulk of the release. The egress keys are the pair worth a second look, since this project also concerns itself with where a request goes, but they gate a sandbox's network policy rather than a session's endpoint, and nothing spawned by injection point #3 is inside that sandbox — so, as with `gatewayInternalNetworks` at 2.1.268, the two do not meet. Nothing in either release duplicates what claudapter adds. 2.1.274 ends that run, and its break is a fourth kind: not a rename, not an argument shape, not a statement added to a body, but the **assigned value ceasing to be a local**. Injection point #3 had matched `<opts>.env=<ident>` since 2.1.220; 2.1.274 assigns a call instead — `Z.env=R;` became `M.env=Ba$(E,H===!0);` — and the signature, which spelled the env as `[w$]+`, matched zero times. The helper is three lines (`function Ba$($,Q){if(!Q)return $;return{...$,CLAUDE_CODE_RESUME_INTERRUPTED_TURN:"1",CLAUDE_CODE_RESUME_INTERRUPTED_TURN_MAX_AGE_MS:String(3600000)}}`) and fires off `spawnClaude`'s new thirteenth parameter, which the new `claudeCode.continueAfterReload` setting drives: after a window reload a restored session continues the step that was interrupted, and the CLI is told so through the environment. The env capture now allows one trailing call with no nesting, `[w$]+(?:([^()]*))?`, which is all any release has needed; a nested call would end the match early and fail loudly, the same trade #10 already carries. `envFor` is handed the helper's result, so those two variables are in the env it filters, and the profile it applies still wins. The other nine signatures matched as written, through a rename of #2's icon pair (`light:W,dark:W` → `light:G,dark:G`). The rest of the release is small and out of reach: `extension.js` grew 37,985 bytes and the webview 16,012, `contributes.commands` holds at 27, VS Code settings go 17 → 19 (`continueAfterReload` and `lockEditorGroups`, both defaulting to on), the CLI settings schema is unchanged but for its `$comment` timestamp (`2026-09-15T17:09:04.693Z` → `2026-09-16T21:45:55.309Z`), and the webview registers 27 actions rather than 25 — the two new ones are `memory` and `instructions`, a dialog over the project's CLAUDE.md files ("Edit CLAUDE.md files", "No memories saved for this project yet.", "Reload Claude"). The Model section's sort order is still the stock five, one hit in the stock backup, so neither new action joined the section injection point #5 reorders. `continueAfterReload` is the one key worth a second look, because claudapter also offers to pick a conversation back up after it stops — but that prompt is armed by an error banner, a hard usage limit or a user interrupt and does nothing but fill the composer, while this one is armed by a window reload and works by handing the CLI an environment variable. They never fire on the same event, and neither can see the other. 2.1.276 costs nothing either, and it is the case both structural anchors exist for: they renamed at once. #3's env helper went `Ba$(E,H===!0)` → `ye$(E,H===!0)`, #2's icon pair `light:G,dark:G` → `light:K,dark:K`, and all fifteen signatures matched the shapes they already matched. 2.1.275 never landed on this machine. The release is editor-side: `contributes.commands` goes 27 → 31 over an inline accept/reject gutter for a proposed edit (`acceptProposedHunk`, `rejectProposedHunk`, `acceptProposedHunkFromBar`, `rejectProposedHunkFromBar`), VS Code settings 19 → 20 (`claudeCode.scrollToBottomOnSend`), and the CLI settings schema holds at 168 top-level keys while gaining `version` and `registry` under each of the three marketplace lists; its `$comment` moved `2026-09-16T21:45:55.309Z` → `2026-09-18T00:43:07.838Z`. `extension.js` grew 67,662 bytes and the webview 51,423. The webview registers the same 27 actions and the Model section's sort order is still the stock five, one hit in the stock backup. The one change worth a second look is a rewind that now restores code as well as the conversation — "Rewind code", a "What to restore" choice group, and "Code rewind completed, but N files were skipped" — which sits next to this project's retract without overlapping it: retract hides the failed exchange and instructs the agent through the session object, while that one rolls files back to a checkpoint. The webview also gained three telemetry names (`banner_shown`, `banner_clicked`, `banner_dismissed`); the six `banner_<hash>` CSS-module classes the auto-resume detector reads are unchanged, so nothing it matches on moved. Everything below comes from the bundle itself: line numbers refer to a formatted `extension.js` (`prettier 3.x --parser babel`, 143,324 lines), signatures to the minified original. Minified identifiers are renamed on nearly every release — they are quoted to make a spot findable, not as stable names.
...
| 2.1.259 | `N.env=F;` | `;` |
| 2.1.265–2.1.266 | `N.env=M;` | `;` |
| 2.1.267 | `O.env=R;` | `;` |
| 2.1.268–2.1.273 | `Z.env=R;` | `;` |
| 2.1.274 | `M.env=Ba$(E,H===!0);` | `;` |
| 2.1.276 | `M.env=ye$(E,H===!0);` | `;` |
...
```js
/let ([\w$]+)=([\w$]+)\.modelSelection\.value,([\w$]+)=[\w$]+\(\2\.claudeConfig\.value\),([\w$]+)=[\w$]+\([^;]*?,\2\.lastServedModel\.value,\3\);([\w$]+)\.commandRegistry\.registerAction\(\{id:"model",label:"Switch model…",description:"Change the AI model",trailingComponent:\4\?([\w$]+)\("span"/
```
...
The first argument to the label helper is `[^;]*?` rather than a back-reference because 2.1.266 put an
expression there: the selection is now resolved through the config — `N_(cfg,sel)?.value??sel` — before
the label is built. That argument is not captured and not re-emitted, so nothing depended on its
spelling; the signature simply named it, and stopped matching when it grew. A minified expression
cannot contain a `;`, so the lazy run cannot overshoot the `);` that ends the statement.
